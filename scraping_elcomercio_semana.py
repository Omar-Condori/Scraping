...
try:
    ...
    for selector in selectores_posibles:
        ...
        if elementos:
            ...
            for elem in elementos[:15]:  # Aumentar límite para más noticias
                ...
                if (enlace and 
                    ...
                    # Filtrar noticias de esta semana
                    if fecha_parseada and inicio_semana.date() <= fecha_parseada <= fin_semana.date():
                        noticias_semana.append(noticia)
                    elif fecha == "hoy" or fecha == "ayer" or "hace" in fecha.lower():
                        # Incluir noticias recientes sin fecha específica
                        noticias_semana.append(noticia)
    ...
    # Filtrar noticias de la semana (sin duplicados)
    noticias_semana_unicas = []
    titulos_semana_vistos = set()
    
    for noticia in noticias_semana:
        titulo_normalizado = re.sub(r'[^\w\s]', '', noticia['titulo'].lower())
        if titulo_normalizado not in titulos_semana_vistos:
            titulos_semana_vistos.add(titulo_normalizado)
            noticias_semana_unicas.append(noticia)
    
    # Si no hay suficientes noticias de la semana, incluir las más recientes
    if len(noticias_semana_unicas) < 5:
        print("⚠️ Pocas noticias con fecha específica, incluyendo noticias recientes...")
        for noticia in noticias_unicas:
            if noticia not in noticias_semana_unicas:
                noticias_semana_unicas.append(noticia)
                if len(noticias_semana_unicas) >= 20:  # Límite de noticias
                    break
    
    # Crear archivo Excel
    wb = Workbook()
    ws = wb.active
    ws.title = "Noticias de la Semana"
    
    # Agregar encabezado
    ws['A1'] = "REPORTE DE NOTICIAS DE LA SEMANA - EL COMERCIO"
    ...
    ws['A4'] = f"Total de noticias encontradas: {len(noticias_semana_unicas)}"
    ...
    for idx, noticia in enumerate(noticias_semana_unicas, 1):
        row = idx + 6
    ...
    for row in range(6, len(noticias_semana_unicas) + 7):
        for col in range(1, 8):
    ...
    categorias_count = {}
    for noticia in noticias_semana_unicas:
        cat = noticia['categoria']
        categorias_count[cat] = categorias_count.get(cat, 0) + 1
    
    # Agregar encabezado de resumen
    ws2['A1'] = "RESUMEN SEMANAL POR CATEGORÍAS"
    ...
    nombre_archivo = f"reporte_semanal_elcomercio_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx"
    wb.save(nombre_archivo)
    
    print(f"\n📊 REPORTE SEMANAL GENERADO EN EXCEL")
    print("=" * 70)
    print(f"📁 Archivo guardado como: {nombre_archivo}")
    print(f"📅 Período: {inicio_semana.strftime('%d/%m/%Y')} - {fin_semana.strftime('%d/%m/%Y')}")
    print(f"📈 Total de noticias de la semana: {len(noticias_semana_unicas)}")
    ...
    for i, noticia in enumerate(noticias_semana_unicas[:10], 1):
        print(f"\n{i}. {noticia['titulo']}")
    ...
    print(f"\n✅ ¡Archivo Excel semanal listo para descargar!")
